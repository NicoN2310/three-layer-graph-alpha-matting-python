...
def get_three_graph(I, trimap, K0, K1, K2, K3, ratio_1=0.418746, ratio_2=0.154445, debug=False):
...
def get_graph(I, trimap, neighborsArray, flag, tol=1e-3):
    ...
    cond_2 = trimap == 0
    cond_3 = flag == 0
    indArray = np.nonzero(~(cond_1 + cond_2 + cond_3).flatten(order="F"))[0].reshape(-1, 1)
    ...
    W = sparse.eye(N, format='csr') - sparse.csr_matrix((w_array, (i_array, j_array)), shape=(N, N), dtype=np.float64)
    W_res = (O.T @ W).tocsr()
